(* Course : COMP 302: Programming Languages and Paradigms
   Copyright © 2018 Brigitte Pientka                     
...
(* ---------------------------------------------------- *)
(* val flatten' : 'a lazy_list fin_list -> 'a fin_list = <fun>
   val flatten : 'a lazy_list lazy_list -> 'a lazy_list =
*)
let rec flatten' s = match s with 
  | Empty -> Empty
  | NonEmpty xs -> NonEmpty (flatten xs)

and flatten xs = append xs.hd (flatten' (force xs.tl))
